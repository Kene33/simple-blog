...
export function PostCard({ post, onLike, onBookmark, onShare, onReport, detail = false }) {
  ...
  const runAction = async (name, action) => { if (pendingAction) return; setPendingAction(name); try { await action?.(); } finally { setPendingAction(""); } };
  const copy = () => runAction("share", async () => { await onShare?.(post); setCopied(true); window.setTimeout(() => setCopied(false), 2200); });
  return <article className="post-card">
    ...
    {detailView ? <div className="post-body-link"><h2>{post.title}</h2><p><LinkifiedText>{post.content}</LinkifiedText></p><Media post={post} onOpen={setMediaIndex} /></div> : <><div className="post-body-link"><Link to={`/posts/${post.id}`}><h2>{post.title}</h2></Link><p><LinkifiedText>{post.content}</LinkifiedText></p></div><div className="post-media-shell"><Media post={post} onOpen={setMediaIndex} /></div></>}
    ...
    <footer className={`post-actions ${detailView ? "post-actions-detail" : ""}`}><button disabled={Boolean(pendingAction)} className={post.liked_by_me ? "liked" : ""} onClick={() => runAction("like", () => onLike(post))} aria-label={post.liked_by_me ? "Убрать лайк" : "Поставить лайк"}><Heart size={19} fill={post.liked_by_me ? "currentColor" : "none"} /> <b>{detailView ? "Нравится" : post.like_count}</b>{detailView && <span>{post.like_count}</span>}</button><Link to={`/posts/${post.id}`}><MessageCircle size={19} /> <b>{detailView ? "Ответить" : post.comment_count}</b>{detailView && <span>{post.comment_count}</span>}</Link><button disabled={Boolean(pendingAction)} onClick={copy} aria-label="Поделиться публикацией"><Send size={19} /> <b>{detailView ? "Копировать" : post.share_count}</b>{detailView && <span>{post.share_count}</span>}</button>{detailView && onReport && <button disabled={Boolean(pendingAction)} onClick={() => onReport()} aria-label="Пожаловаться на публикацию"><Flag size={17} /> <b>Жалоба</b></button>}<button disabled={Boolean(pendingAction)} className={`bookmark ${post.bookmarked_by_me ? "bookmarked" : ""}`} onClick={() => runAction("bookmark", () => onBookmark(post))} aria-label={post.bookmarked_by_me ? "Убрать из закладок" : "Добавить в закладки"}><Bookmark size={19} fill={post.bookmarked_by_me ? "currentColor" : "none"} /></button></footer>
