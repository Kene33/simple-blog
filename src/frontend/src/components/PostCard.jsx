...
export function PostCard({ post, onLike, onBookmark, onShare, onReport, detail = false }) {
  ...
  return <article className="post-card">
    ...
    {detail ? <div className="post-body-link"><h2>{post.title}</h2><p>{post.content}</p>{post.media?.length > 0 && <div className={`post-media post-media-${Math.min(post.media.length, 2)}`}>{post.media.slice(0, 2).map((media) => media.kind === "video" ? <video key={media.id} controls src={media.url} /> : <img key={media.id} src={media.url} alt="Вложение публикации" />)}<span className="media-count"><Paperclip size={13} /> {post.media.length} вложения</span></div>}</div> : <Link to={`/posts/${post.id}`} className="post-body-link"><h2>{post.title}</h2><p>{post.content}</p>{post.media?.length > 0 && <div className={`post-media post-media-${Math.min(post.media.length, 2)}`}>{post.media.slice(0, 2).map((media) => media.kind === "video" ? <video key={media.id} controls src={media.url} /> : <img key={media.id} src={media.url} alt="Вложение публикации" />)}<span className="media-count"><Paperclip size={13} /> {post.media.length} вложения</span></div>}</Link>}
    <div className="tags">{post.tags.map((tag) => <span key={tag}>#{tag}</span>)}</div>
    <footer className={`post-actions ${detail ? "post-actions-detail" : ""}`}><button className={post.liked_by_me ? "liked" : ""} onClick={() => onLike(post)}><Heart size={19} fill={post.liked_by_me ? "currentColor" : "none"} /> <b>{detail ? "Нравится" : post.like_count}</b>{detail && <span>{post.like_count}</span>}</button><Link to={`/posts/${post.id}`}><MessageCircle size={19} /> <b>{detail ? "Ответить" : post.comment_count}</b>{detail && <span>{post.comment_count}</span>}</Link><button onClick={() => onShare?.(post)} aria-label="Поделиться публикацией"><Send size={19} /> <b>{detail ? "Копировать" : post.share_count}</b>{detail && <span>{post.share_count}</span>}</button>{detail && <button onClick={() => onReport?.()}><Flag size={17} /> <b>Жалоба</b></button>}<button className={`bookmark ${post.bookmarked_by_me ? "bookmarked" : ""}`} onClick={() => onBookmark(post)}><Bookmark size={19} fill={post.bookmarked_by_me ? "currentColor" : "none"} /></button></footer>
  </article>;
}
