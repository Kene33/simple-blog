...
function MessageBubble({ message, own, onEdit, onDelete }) {
  ...
  return <div className={`message-row ${own ? "own" : ""}`}><div className={`message-bubble ${deleted ? "deleted" : ""}`}>
    ...
    {menu && <div className="message-menu"><button onClick={() => { setEditing(true); setMenu(false); }}>Изменить</button><button onClick={() => { onDelete(message.id); setMenu(false); }}>Удалить</button></div>}
  </div></div>;
}
...
function ConversationView({ conversation, currentUser, onBack, onChanged }) {
  ...
  useEffect(() => {
    ...
    api.conversationMessages(conversation.id, { limit: 30 }).then((page) => { if (cancelled) return; setMessages(page.items || []); setCursor(page.next_cursor); setState("ready"); api.markConversationRead(conversation.id).catch(() => {}); }).catch((error) => setState(errorStatus(error) === 404 ? "soon" : "error"));
  ...
  const send = async (event) => { event.preventDefault(); const value = body.trim(); if (!value || busy) return; setBusy(true); const clientId = crypto.randomUUID?.() || `${Date.now()}`; try { const message = await api.sendMessage(conversation.id, { body: value, client_id: clientId }); setMessages((items) => addUnique(items, message)); setBody(""); } catch { } finally { setBusy(false); } };
  const loadOlder = async () => { if (!cursor || busy) return; setBusy(true); try { const page = await api.conversationMessages(conversation.id, { before: cursor, limit: 30 }); setMessages((items) => [...(page.items || []), ...items]); setCursor(page.next_cursor); } finally { setBusy(false); } };
  const edit = async (id, value) => { try { const updated = await api.updateMessage(id, { body: value }); setMessages((items) => items.map((item) => item.id === id ? updated : item)); } catch { } };
  const remove = async (id) => { if (!window.confirm("Удалить сообщение?")) return; try { const updated = await api.deleteMessage(id); setMessages((items) => items.map((item) => item.id === id ? (updated || { ...item, is_deleted: true, body: "" }) : item)); } catch { } };
  return <section className="conversation-view"><header><button className="messages-back" onClick={onBack} aria-label="Назад к диалогам"><ArrowLeft size={18} /></button><Avatar user={other} /><div><b>{other.display_name || other.username || "Пользователь"}</b><small>@{other.username || ""}</small></div><span className="socket-state">{socketState === "connected" ? <Wifi size={14} /> : <WifiOff size={14} />}{socketState === "connected" ? "В сети" : socketState === "offline" ? "Офлайн" : "Переподключение…"}</span></header>{state === "soon" ? <SoonState /> : state === "error" ? <div className="messages-state">Не удалось загрузить сообщения</div> : state === "loading" ? <div className="messages-state"><LoaderCircle className="spin" size={19} /> Загружаем…</div> : <><div className="message-history">{cursor && <button className="load-older" onClick={loadOlder}>Загрузить предыдущие</button>}{messages.map((message) => <MessageBubble key={message.id || message.client_id} message={message} own={message.author_id === currentUser?.id || message.sender_id === currentUser?.id} onEdit={edit} onDelete={remove} />)}{!messages.length && <div className="messages-empty">Начните разговор</div>}<span ref={endRef} /></div><form className="message-composer" onSubmit={send}><textarea value={body} onChange={(event) => setBody(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); event.currentTarget.form.requestSubmit(); } }} maxLength={4000} placeholder="Напишите сообщение…" aria-label="Сообщение" /><button className="primary" disabled={busy || !body.trim()} aria-label="Отправить сообщение"><Send size={17} /></button></form></>}</section>;
}
