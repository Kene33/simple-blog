...
export function FeedPage() {
  ...
  return <AppShell title="Лента" right={<RightRail />}><section className="feed-page"><header className="page-title"><div><h1>Лента</h1><p>Идеи и обсуждения сообщества</p></div><button className="round-button" onClick={() => load()} aria-label="Обновить ленту"><RefreshCw size={20} /></button></header>
    ...
    {posts.map((post) => <PostCard key={post.id} post={post} onLike={(post) => toggle(post, "like")} onBookmark={(post) => toggle(post, "bookmark")} />)}{cursor && <button className="outline-button load-more" disabled={state === "loading-more"} onClick={() => load(true)}>{state === "loading-more" ? "Загружаем…" : "Показать ещё"}</button>}
  </section></AppShell>;
}
