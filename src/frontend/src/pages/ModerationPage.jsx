...
export function ModerationPage() {
  ...
  return <AppShell title="Модерация"><section className="moderation-page">
    ...
    {state === "loading" && !reports.length ? <div className="card-state">Загружаем обращения…</div> : state === "error" ? <div className="card-state">Не удалось загрузить очередь</div> : <><div className="reports-table"><div className="report-row report-titles"><span>Жалоба</span><span>Репортёр</span><span>Объект</span><span>Причина</span><span>Создана</span></div>{reports.map((report) => <div className="report-row" key={report.id}><span><i className={`report-status ${report.status}`}>{report.status}</i><b>#{report.id.slice(0, 8)}</b></span><span>@{report.reporter.username}</span><span>{report.target?.kind === "post" ? <Link to={`/posts/${report.post_id}`}>Пост</Link> : "Комментарий"}<small>{report.target?.is_deleted ? "Удалён" : "Открыть объект"}</small></span><span><b>{reasonLabels[report.reason] || report.reason}</b><small>{report.details || report.resolution || "Без деталей"}</small></span><span>{new Date(report.created_at).toLocaleDateString("ru-RU")}</span><button className="outline-button" onClick={() => open(report.id)}><Eye size={15} /> Рассмотреть</button></div>)}{!reports.length && <div className="empty-row">Очередь пуста</div>}</div>{nextCursor && <button className="outline-button next-page" onClick={() => setCursor(nextCursor)}>Следующая страница</button>}</>}
    {selected && <div className="modal-backdrop" role="presentation"><section className="moderation-modal" role="dialog" aria-modal="true"><button className="modal-close" onClick={() => setSelected(null)} aria-label="Закрыть"><X /></button><small>ЖАЛОБА НА {selected.target.kind.toUpperCase()}</small><h2>#{selected.id.slice(0, 8)}</h2><p><b>{reasonLabels[selected.reason] || selected.reason}</b> · @{selected.reporter.username}</p><blockquote>{selected.target.title || selected.target.body}</blockquote><footer><button className="outline-button" onClick={() => resolve("rejected")}>Отклонить</button><button className="danger-button" onClick={() => resolve("resolved")}>Решить жалобу</button></footer></section></div>}
  </section></AppShell>;
}
