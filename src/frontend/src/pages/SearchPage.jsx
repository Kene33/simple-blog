...
export function SearchPage() {
  ...
  useEffect(() => { const delay = setTimeout(() => { const params = new URLSearchParams(); if (query.trim()) params.set("q", query.trim()); if (searchIn !== "all") params.set("search_in", searchIn); replace(`/search${params.size ? `?${params}` : ""}`); if (!query.trim()) { setPosts([]); return setState("empty"); } setState("loading"); api.posts({ query, search_in: searchIn }).then((page) => { setPosts(page.items); setState(page.items.length ? "ready" : "empty"); }).catch(() => { setPosts([]); setState("error"); }); }, 300); return () => clearTimeout(delay); }, [query, searchIn]);
