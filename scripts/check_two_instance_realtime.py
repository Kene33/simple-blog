...
async def register_device(client: httpx.AsyncClient, label: str) -> dict[str, object]:
    response = await client.post("/api/v1/messaging/devices", json={"public_key": {"kty": "EC", "crv": "P-256", "x": label, "y": "key"}, "label": label}, headers={"X-CSRF-Token": client.cookies.get("csrf_token")})
...
async def main() -> None:
    ...
    async with httpx.AsyncClient(base_url=instance_a) as client_a, httpx.AsyncClient(base_url=instance_b) as client_b:
        ...
        async with websockets.connect(ws_url, origin=instance_b, additional_headers={"Cookie": f"access_token={access_cookie}"}) as socket:
            ...
            sent = await client_a.post(f"/api/v1/conversations/{conversation_id}/messages", json={"envelope": envelope}, headers={"X-CSRF-Token": client_a.cookies.get("csrf_token")})
